...
const NotificationsPage: React.FC = () => {
  ...
  useEffect(() => {
    // When leaving page, we could mark all as read, 
    // but for now let's just let the user see them.
  }, []);
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'offer_received': return <Send size={20} style={{ color: 'var(--primary)' }} />;
      case 'offer_approved': return <CheckCircle2 size={20} style={{ color: 'var(--success)' }} />;
  ...
  return (
    <div className="container" style={{ padding: '2rem 1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem' }}>通知</h1>
        {myNotifications.some(n => !n.read) && (
          <button 
            onClick={clearNotifications}
            style={{ background: 'none', color: 'var(--primary)', fontSize: '0.875rem', fontWeight: 600 }}
          >
            すべて既読にする
          </button>
        )}
        ...
        {myNotifications.length > 0 ? myNotifications.map(notif => (
          <Link 
            ...
            style={{ 
              ...
              backgroundColor: notif.read ? 'white' : 'rgba(79, 70, 229, 0.03)',
